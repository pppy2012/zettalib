...
bool PGConnection::Connect() {
  ...
  snprintf(conninfo, 1024,
           "host=%s port=%d user=%s password=%s connect_timeout=%d dbname=%s",
           host.c_str(), pg_connection_option_.port_num,
           pg_connection_option_.user.c_str(),
           pg_connection_option_.password.c_str(),
...
bool PGConnection::SetAutoCommit() { return true; }

int PGConnection::ExecuteQuery(const char *sql_stmt, PgResult *pgResult,
                               bool force_retry) {
  if (!CheckIsConnected())
    return -1;
  ...
  if (status == PGRES_COMMAND_OK) {
    ...
    if (strcmp(errnum, "08006") == 0 || strcmp(errnum, "08003") == 0) {
      if (force_retry) {
        Reconnect();
        return ExecuteQuery(sql_stmt, pgResult, false);
      }
      setErr("execute query sql: %s failed, %s, %s", sql_stmt, errnum,
